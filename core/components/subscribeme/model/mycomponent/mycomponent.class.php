...
class SubscribeMe {
    ...
    */
    ...
    /**
     * @var string Path to SubscribeMe Core
     */
    protected $corePath;
    /**
     * @var string Path to SubscribeMe assets directory
     */
    protected $assetsPath;
    /**
     * @var string URL to SubscribeMe Assets directory
     */
    protected $assetsUrl;
    /**
    ...
    public function __construct(&$modx, &$props) {
        ...
        $this->corePath = $this->modx->getOption('np.core_path', null, MODX_CORE_PATH . 'components/subscribeme/');
        $this->assetsPath = $this->modx->getOption('np.assets_path', null, MODX_ASSETS_PATH . 'components/subscribeme/');
        $this->assetsUrl = $this->modx->getOption('np.assets_url', null, MODX_ASSETS_URL . 'components/subscribeme/');
    }
     ...
     */
        ...
        public function init($context) {
            ...
            switch ($context) {
                ...
                default:
                    $language = !empty($this->props['language'])
                            ? $this->props['language'] . ':' : '';
                    $this->modx->lexicon->load($language . 'subscribeme:default');
                    break;
            }
           ...
           if (empty($this->props['cssfile'])) { /* nothing sent - use default */
               $css = $this->assetsUrl . 'css/subscribeme.css';
           } elseif (empty($this->props['cssfile']) ) { /* empty param -- no css file */
               $css = false;
           } else {  /* set but not empty -- use it */
               $css = $this->assetsUrl . 'components/subscribeme/css/' . $this->props['cssfile'];
           }
    ...
    public function getTpls() {
        ...
        if (empty($this->tpls['outerTpl'])) {
            $this->tpls['outerTpl'] = '<div class="subscribeme">
